...
export function PlayerModal({ item, credentials, onClose }: PlayerModalProps) {
  ...
  useEffect(() => {
    ...
    const isHls = ext === "m3u8" || url.toLowerCase().endsWith(".m3u8");
    ...
    function attachNativeFallback() {
      ...
      video.addEventListener(
        "error",
        (ev) => {
          const msg = (video.error?.message || "Formato não suportado pelo navegador.");
          setErrorMsg(`Não foi possível reproduzir. ${msg}`);
          setStatus("error");
        },
        { once: true }
      );
    ...
    if (isHls) {
      if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (_e, data) => {
          if (data.fatal) {
            if (item.kind !== "live" && !url.includes(".m3u8")) {
              attachNativeFallback();
              return;
            }
            switch (data.type) {
              case Hls.ErrorTypes.NETWORK_ERROR:
                setErrorMsg("Erro de rede. O canal pode estar offline ou o servidor bloqueou a conexão.");
                break;
